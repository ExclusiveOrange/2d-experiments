...
namespace raycasting::shapes
{
  ...
  makeCone(glm::vec3 diffuse, float height, float radiusAtHeight) // height can be negative
  {
    ...
    const float sqradius = sq(radiusAtHeight / height);
    const float minz = height < 0 ? height : 0.f;
    const float maxz = height < 0 ? 0.f : height;
    const float zslope = -radiusAtHeight / height;
