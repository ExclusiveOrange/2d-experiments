...
namespace
{
  ...
  namespace raycasting
  {
    ...
    struct Light
    {
      virtual glm::vec3 calculate(glm::vec3 position, glm::vec3 normal) const = 0;
    };
    ...
    struct DirectionalLight : Light
    {
      ...
      glm::vec3 calculate(glm::vec3 position, glm::vec3 normal) const override
      {
        ...
        for (int x = 0; x < destImage.w; ++x)
        {
          ...
          if (std::optional<Intersection> i = intersectable.intersect(ray))
          {
            ...
            for (int iLight = 0; iLight < numLights; ++iLight)
              lightSum += lights[iLight].calculate(i->position, i->normal);
